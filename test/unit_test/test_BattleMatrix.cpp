...
int main()
{
    ...
    get_battle_score(pokemon_lucario, pokemon_giratina_altered, 0, 0);

    std::cout << "success" << std::endl;

    std::cout << "testing battle matrix ... ";

    std::vector<PvPPokemon> pkm_list{
        pokemon_groudon,
        pokemon_dialga,
        pokemon_lucario,
    ...
    BattleMatrix bm;
    bm.set(pkm_list, pkm_list, false);
    bm.run();

    auto start = std::chrono::high_resolution_clock::now();

    Matrix_t matrix = bm.get();
