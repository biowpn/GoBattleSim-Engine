...
def main():
    ...
    for src in os.listdir(SourceDir):
        ...
        cmd = f"em++ --std=c++11 -O3 -Iinclude -Ithirdparty -c {src_path} -o {bin_path}"
    ...
    cmd = f"emcc {bin_paths_str} -o {BinDir}/GBS_Engine.html -s EXTRA_EXPORTED_RUNTIME_METHODS=[ccall,cwrap]"
    print(cmd)
    p = subprocess.Popen(cmd, shell=True)
    p.wait()
